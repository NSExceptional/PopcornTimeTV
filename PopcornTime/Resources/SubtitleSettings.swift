...
class SubtitleSettings: NSObject, NSCoding {
    ...
    override init() {
        guard let codedData = UserDefaults.standard.data(forKey: "subtitleSettings"), let settings = try? NSKeyedUnarchiver.unarchivedObject(ofClass: SubtitleSettings.self, from: codedData) else { return }
